...
def matmul_batch_kernel(a_ref, b_ref, o_ref, *, activation):
  """
...
def run_batch_matmul(
  ...
  kernel = pl.pallas_call(
      functools.partial(matmul_batch_kernel, activation=activation),
      grid=(N, N),
      in_specs=[
...
def batch_matmul_with_activation(
  ...
  """
  return jax.vmap(
      functools.partial(run_batch_matmul, N=N, activation=activation),
      in_axes=(0, 0)
  )(a, b)
